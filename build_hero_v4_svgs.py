...
def create_stunning_svgs():
    # 1. Read and base64 encode the 600x600 resized avatar image
    avatar_path = "avatar_resized.jpg"
    with open(avatar_path, "rb") as img_file:
        b64_avatar = base64.b64encode(img_file.read()).decode("utf-8")

    # 2. ASCII Matrix Text Overlay
    ascii_matrix = [
        r"01001001 01001110 01010100 01000101 01001100 01001100",
        r" .:  [SYS]: UJJWAL_KERNEL_v5.4 // ACTIVE       :. ",
    ...
    for i, line in enumerate(ascii_matrix):
        ...
        ascii_matrix_tspans += f'<tspan x="54" dy="14">{escaped}</tspan>\n'
  ...
  <defs>
    ...
    <linearGradient id="scanlineGrad" x1="0%" y1="0%" x2="0%" y2="100%">
      ...
      <stop offset="50%" stop-color="{grad_c2}" stop-opacity="0.35"/>
  ...
  <!-- ========================================================= -->
  <!-- LEFT PANEL: CYBER TERMINAL & HIGH-END MALE AVATAR IMAGE   -->
  <!-- ========================================================= -->
  <g id="left-panel" filter="url(#cardShadow)">
    ...
    <g clip-path="url(#leftCardClip)">
      ...
      <!-- Header Title -->
      <text x="242" y="56" text-anchor="middle" fill="{text_secondary}" font-family="'JetBrains Mono', monospace" font-size="11" font-weight="600">developer_avatar.sh — ujjwal@dev</text>
    ...
    <!-- EMBEDDED HIGH-END 3D MALE DEVELOPER AVATAR IMAGE -->
    <g clip-path="url(#insetClip)">
      ...
      <g>
        ...
        <image href="data:image/jpeg;base64,{b64_avatar}" x="46" y="84" width="392" height="480" preserveAspectRatio="xMidYMid slice"/>

        <!-- Semi-transparent Dark Tint Overlay for Cyber Look -->
        <rect x="46" y="84" width="392" height="480" fill="{ 'rgba(3, 7, 18, 0.22)' if is_dark else 'rgba(15, 23, 42, 0.10)' }"/>

        <!-- Cyber Reflection Diagonal Highlight -->
        <path d="M 46 84 L 220 84 L 46 258 Z" fill="rgba(255,255,255,0.06)"/>

        <!-- LAYERED ASCII MATRIX CODE OVERLAY -->
        <g id="ascii-matrix-overlay" filter="url(#pillGlow)">
          <text x="54" y="106" fill="url(#asciiGrad)" font-family="'JetBrains Mono', 'Fira Code', monospace" font-size="9px" letter-spacing="0.2px">
            <animate attributeName="opacity" values="0.15; 0.45; 0.20; 0.50; 0.15" dur="7s" repeatCount="indefinite"/>
